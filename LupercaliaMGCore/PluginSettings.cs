...
public class PluginSettings
{
     ...
     */
    public readonly FakeConVar<bool> m_CVIsRoundEndDamageImmunityEnabled = new("lp_mg_redi_enabled",
        "Should player grant damage immunity after round end until next round starts.", false);

    public readonly FakeConVar<bool> m_CVIsRoundEndWeaponStripEnabled = new("lp_mg_rews_enabled",
        "Should player's weapons are removed before new round starts.", true);

    public readonly FakeConVar<bool> m_CVIsRoundEndDeathMatchEnabled =
        new("lp_mg_redm_enabled", "Should enable round end death match?", true);

    /*
     *   Scheduled shutdown
     */
    public readonly FakeConVar<string> m_CVScheduledShutdownTime = new("lp_mg_scheduled_shutdown_time",
        "Server will be shutdown in specified time. Format is HHmm", "0500");

    public readonly FakeConVar<int> m_CVScheduledShutdownWarningTime = new("lp_mg_scheduled_shutdown_warn_time",
        "Show shutdown warning countdown if lp_mg_scheduled_shutdown_round_end is false.", 10);

    public readonly FakeConVar<bool> m_CVScheduledShutdownRoundEnd = new("lp_mg_scheduled_shutdown_round_end",
        "When set to true server will be shutdown after round end.", true);

    /*
     *   Auto respawn
     */
    public readonly FakeConVar<bool> m_CVAutoRespawnEnabled =
        new("lp_mg_auto_respawn_enabled", "Auto respawn feature is enabled", false);

    public readonly FakeConVar<float> m_CVAutoRespawnSpawnKillingDetectionTime =
        new("lp_mg_auto_respawn_repeat_kill_time", "Seconds to detect as spawn killing.", 1.0F);

    public readonly FakeConVar<float> m_CVAutoRespawnSpawnTime =
        new("lp_mg_auto_respawn_time", "How long to respawn after death.", 1.0F);

    /*
     *   Anti camp
     */
    public readonly FakeConVar<bool>
        m_CVAntiCampEnabled = new("lp_mg_anti_camp_enabled", "Anti camp enabled", true);

    public readonly FakeConVar<float> m_CVAntiCampDetectionTime = new("lp_mg_anti_camp_detection_time",
        "How long to take detected as camping in seconds.", 10.0F);

    public readonly FakeConVar<double> m_CVAntiCampDetectionRadius = new("lp_mg_anti_camp_detection_radius",
        "Range of area for player should move for avoiding the detected as camping.", 200.0F);

    public readonly FakeConVar<float> m_CVAntiCampDetectionInterval = new("lp_mg_anti_camp_detection_interval",
        "Interval to run camping check in seconds.", 0.1F);

    public readonly FakeConVar<float> m_CVAntiCampMarkingTime = new("lp_mg_anti_camp_glowing_time",
        "How long to detected player keep glowing.", 10.0F);

    /*
     *   Map config
     */
    public readonly FakeConVar<int> m_CVMapConfigExecutionTiming = new("lp_mg_mapcfg_execution_timing",
    ...
    public readonly FakeConVar<int> m_CVMapConfigType = new("lp_mg_mapcfg_type",
     ...
     *   Omikuji
     */
    public readonly FakeConVar<int> m_CVOmikujiEventWeightMisc = new("lp_mg_omikuji_event_weight_misc",
        "Weight of misc event. You can set to any value.", 90);

    public readonly FakeConVar<int> m_CVOmikujiEventWeightBad = new("lp_mg_omikuji_event_weight_bad",
        "Weight of bad event. You can set to any value.", 5);

    public readonly FakeConVar<int> m_CVOmikujiEventWeightLucky = new("lp_mg_omikuji_event_weight_lucky",
        "Weight of lucky event. You can set to any value.", 5);

    public readonly FakeConVar<double> m_CVOmikujiCommandCooldown =
        new("lp_mg_omikuji_command_cooldown", "Cooldown of omikuji command.", 60.0D);

    public readonly FakeConVar<int> m_CVOmikujiCommandExecutionDelayMin = new(
        "lp_mg_omikuji_command_execution_delay_min",
        "Minimum time of omikuji event executed after execution of command.", 5);

    public readonly FakeConVar<int> m_CVOmikujiCommandExecutionDelayMax = new(
     ...
     *   Omikuji - Chicken
     */
    public readonly FakeConVar<float> m_CVOmikujiEventChickenTime =
        new("lp_mg_omikuji_event_chicken_time", "How long that chicken alive?", 5.0F);

    public readonly FakeConVar<float> m_CVOmikujiEventChickenBodyScale =
        new("lp_mg_omikuji_event_chicken_body_scale", "Body size of the chicken. Default size is 1.0", 1.0F);

    public readonly FakeConVar<double> m_CVOmikujiEventChickenSelectionWeight =
        new("lp_mg_omikuji_event_chicken_selection_weight", "Selection weight of this event", 160.0D);

    /*
     *   Omikuji - Screen shake
     */
    public readonly FakeConVar<float> m_CVOmikujiEventScreenShakeAmplitude = new(
    ...
    public readonly FakeConVar<float> m_CVOmikujiEventScreenShakeDuration = new(
        "lp_mg_omikuji_event_screen_shake_duration", "The length of time in which to shake the player's screens.",
        5.0F);

    public readonly FakeConVar<float> m_CVOmikujiEventScreenShakeFrequency = new(
